...
-- image size.
sinkImageSize :: (Monad m, PipeInput m ~ S.ByteString, IsPipe m) => m (Maybe Size)
...
sinkImageInfo :: (Monad m, IsPipe m, PipeInput m ~ S.ByteString) => m (Maybe (Size, FileFormat))
sinkImageInfo =
    start id
  where
...
getInt :: (Monad m, Integral i, PipeInput m ~ S.ByteString, IsPipe m)
       => Int
       -> i
       -> m (Maybe i)
